...
export default function FlightSearchComponent({ page }: any) {
  ...
  return (
    <div className={`w-full flex items-center justify-center pt-20`}>
      <div className=" xl:w-[1220px] 2xl:w-[1420px]  bg-[#F8F9FE] rounded-md">
              ...
              <div className=" xl:mt-0 mt-16 w-full xl:w-[850px] p-5 rounded-xl bg-[#F8F9FE] items-center grid grid-cols-5 gap-3 sm:gap-0 ">
                <div className=" col-span-5 sm:col-span-2 border-b sm:border-b-0 pb-3 sm:pb-0 sm:border-r border-gray-300 flex items-center sm:justify-center gap-5 flex-wrap">
                  <p className=" font-semibold">
                    {flightFrom.address.cityName} ({flightFrom.address.cityCode}
                    )
                  </p>
                  <div
                    // onClick={switchTravelData}
                    className=" bg-primary h-8 w-8 rounded-full flex items-center justify-center cursor-pointer shadow"
                  >
                    <PiArrowsLeftRightFill className=" text-white font-semibold text-lg" />
                  </div>
                  <p className=" font-semibold">
                    {flightTo.address.cityName} ({flightTo.address.cityCode})
                  </p>
                </div>
                <div className=" col-span-5 sm:col-span-2 border-b sm:border-b-0 pb-3 sm:pb-0 sm:border-r border-gray-300 flex items-center sm:justify-center gap-2">
                  <FaCalendarDays />
                  {flightType === "one-way" && (
                    <p className=" ">{ShortDate(new Date(flightSingleDate))}</p>
                  )}
                  {flightType === "round-trip" && (
                    <p className=" ">
                      {formatDate(new Date(flightSingleDate))} -{" "}
                      {formatDate(new Date(flightReturnDate))}
                    </p>
                  )}
                </div>
        ...
        <div className=" flex items-center justify-between flex-wrap gap-5 p-5 border-gray-300">
          {searchFromString &&
            searchToString &&
            travelers &&
            flightClass &&
              ...
              <div className=" flex items-center flex-wrap gap-5">
                ...
                <div>
                  <p className=" font-medium text-lg">
                    {searchFromString} to {searchToString}
                  </p>
                  <p className=" mt-2 text-sm text-gray-800">
                    {" "}
                    {flightSingleDate.toLocaleDateString("en-US", {
                      weekday: "short",
                      day: "2-digit",
                      month: "short",
                    })}{" "}
                    | {travelers.adults + travelers.child + travelers.infant}{" "}
                    Passenger |{" "}
          ...
          <div
            onClick={() => setShowSearchComponent(!showSearchComponent)}
            className=" flex items-center gap-2 bg-primary bg-opacity-20 text-primary hover:bg-opacity-100 hover:text-white p-3 rounded-md cursor-pointer"
          >
