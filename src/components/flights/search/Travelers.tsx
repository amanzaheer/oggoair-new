...
export default function Travelers({
  travelers,
  setTravelers,
  setShowTravelersSelection,
  option,
}: {
  travelers: TravelersType;
  setTravelers: Function;
  setShowTravelersSelection: Function;
  option: string;
  flightClass: string;
  ...
  return (
    <div
      id="travelers"
      className="donotclose absolute top-full translate-y-2 w-[240px] p-1  bg-white shadow-md rounded z-50"
    >
      {option === "hotel" && (
        <div className=" flex items-center justify-between p-3">
          <div>
            <p className=" text-gray-700 font-light text-sm">Rooms</p>
      ...
      <div className=" flex items-center justify-between p-3 ">
        ...
        <div className=" flex items-center gap-3 p-[2px] border border-blue-300 rounded-md">
          ...
          >
            <FaMinus
              className={`${
                travelers.adults > 1 ? "text-gray-900" : "text-gray-500"
              }`}
            />
          </div>
      ...
      <div className=" flex items-center justify-between p-3">
        ...
        <div className=" flex items-center gap-3 p-[2px] border border-blue-300 rounded-md">
          ...
          >
            <FaMinus
              className={`${
                travelers.child > 0 ? "text-gray-900" : "text-gray-500"
              }`}
            />
          </div>
          <p>{travelers.child}</p>
        ...
        <div className=" flex items-center gap-3 p-[2px] border border-blue-300 rounded-md">
          <div
            ...
            className={`h-7 w-7 rounded flex items-center justify-center group ${
              travelers.infant > 0 ? "text-gray-900" : "text-gray-500"
            } `}
          >
            <FaMinus
              className={`${
          ...
          <div
            ...
            className={`h-7 w-7 rounded flex items-center justify-center `}
          >
            <FaPlus className={`text-primary group-hover:text-white`} />
          </div>
        </div>
      </div>
    </div>
